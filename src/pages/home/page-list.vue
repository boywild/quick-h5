    ...
    <el-scrollbar class="scroll-wrapper page-list-wrapper">
      <div class="page-content">
        <div class="my-page-nav-list">
          <div class="my-page-nav-item" :class="{ active: searchParams.type === 'my' }" @click="doSearch('my')">
            我的作品({{ myCount }})
          </div>
          <div class="my-page-nav-item" :class="{ active: searchParams.type === 'cooperation' }" @click="doSearch('cooperation')">
            参与作品({{ shareCount }})
          </div>
        </div>
        <!--页面列表-->
        <div v-loading="loading" class="page-item-wrapper">
          <div class="page-item">
            <thumbnailPanel :page-type="searchParams.pageMode" />
          </div>
          <div v-for="(item, index) in pageList" :key="index" class="page-item">
            <thumbnailPanel
              :page-data="item"
              :btn-list="operationBtn(item.isPublish)"
              @refresh="getPageList"
